...
- **Free-threading support (Beta).** Adds the
  ...
  free-threaded CPython `3.14t` with the GIL disabled. Four real-thread stress
  tests (marked `pytest.mark.stress`) exercise the Lock/Semaphore-based
  resilience components — `RetryBudget`, `CircuitBreaker`/
  `AsyncCircuitBreaker`, `Bulkhead`/`AsyncBulkhead` — and the shared `httpx2`
  connection pool, with zero cross-talk or crashes under parallelism; a fifth,
